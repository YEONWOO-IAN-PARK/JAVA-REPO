...
/**
 * 사용자 관련 서비스(신규가입, 내정보 조회, 비밀번호 변경,  탈퇴, 포인트변경이력 조회)를 제공한다.
 * @author choes
 *
 */
public class UserService {
	 ...
	 * 탈퇴한 사용자
	 */
	public static final String DISABLED_USER_YES = "Y";
	public static final String DISABLED_USER_NO = "N";
	...
	public void registerUser(User user) throws SQLException {
		// 동일한 아이디로 가입한 사용자가 있는지 확인 --> select * from sample_users where user_id = ?
		// 동일한 이메일 정보로 이미 가입한 사용자가 있는지 확인 --> select * from sample_users where user_email = ?
		...
		if(savedUser != null) {
			throw new UserAlreadyExistException("["+user.getEmail()+"]은 이미 다른 사용자가 사용중인 이메일입니다.");
		}
	...
	/**
	 * 지정된 아이디에 해당하는 사용자정보를 조회한다.
	 * @param userId 사용자 아이디
	 * @return 사용자 정보, 아이디가 올바르지 않은 경우엔..
	 * @throws SQLException
	 */
	public User getUserInfo(String userId) throws SQLException {
		// 아이디로 사용자정보 조회하기 --> select * from sample_users where user_id = ?
	...
	public void changePassword(String userId, String oldPassword, String newPassword) throws SQLException {
		...
		// 아이디로 사용자정보 조회 --> select * from sample_users where user_id = ? UserDao에 구현되어있음.							
		// 조회된 사용자정보의 비밀번호와 사용자가 입력한 이전 비밀번호가 일치하는지 확인, 일치하지않으면 예외발생 PasswordMismatchException
		// 위의 모든과정을 통과했으면 사용자의 비밀번호를 변경하기 - update set....
		...
		if (user == null) {
			throw new UserNotFoundException("["+"] 아이디에 해당하는 사용자를 찾을 수 없습니다.");
		}
	...
	public void disabledUser(String userId, String password) throws SQLException {
		...
		if(DISABLED_USER_YES.equals(user.getDisabled())) {
			throw new UserNotFoundException("["+userId+"] 는 이미 탈퇴처리된 사용자입니다.");
		}
		...
		user.setDisabled(DISABLED_USER_YES);
		userDao.updateUser(user);
	...
	public List<PointHistory> getMyPointHistories(String userId) throws SQLException {
		List<PointHistory> pointHistory = pointHistoryDao.getPointHistoriesByUserId(userId);
		if(pointHistory == null) {
			throw new UserNotFoundException("["+userId+"] 에 해당하는 포인트변경이력을 찾을 수 없습니다.");
		}
